    ...
    <div id="wrapper">
        <nav class="navbar navbar-dark align-items-start sidebar sidebar-dark accordion bg-gradient-primary p-0">
            <div class="container-fluid d-flex flex-column p-0">
                ...
                <ul class="nav navbar-nav text-light" id="accordionSidebar">
                    ...
                    <li class="nav-item" role="presentation"><a class="nav-link active" href="Device List"><i class="fa fa-list"></i><span>Device List</span></a></li>
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            ...
            <div class="container-fluid">
                <h3 class="text-dark mb-3">Device List</h3>
...
if(isset($_POST['data_delete_btn'])){
	...
	// print_r($DB_TRAIN_DATA);
	
	foreach($DB_DEVICE_DATA as $DEVICE_DATA){
	
		$DELETE_DATA_QUERY = mysqli_query($conn,"DELETE FROM `device` WHERE `device`.`Device_ID` = '$DEVICE_DATA'");
	
		if($DELETE_DATA_QUERY === true ){
			$_SESSION['DEL_QUERY_MSG'] = $TRAIN_DATA . " Successfully deleted";
		}else{
			$_SESSION['DEL_QUERY_MSG'] = $TRAIN_DATA . " Can not be deleted. ERROR: " .  mysqli_error($conn);
		}
	}
...
?>			                        
		...
		<script language="JavaScript">
			...
			checkboxes[i].checked = source.checked;
													...
													<th style="text-align: center;">Min Threshold Current (A)</th>
                                                                               			        <th style="text-align: center;">Max Threshold Current (A)</th>
													<th style="text-align: center;">Set Threshold Current</th>			
		...
		$ITEMS_QUERY = mysqli_query($conn,"SELECT * FROM `device`");
		while($ITEMS = mysqli_fetch_assoc($ITEMS_QUERY)){
				
			$DB_DEVICE_ID = $ITEMS['Device_ID'];
			$DB_DEVICE_LOCATION = $ITEMS['Location'];        
			$DB_DEVICE_THRESHOLD_CURRENT_MIN = $ITEMS['Device_Threshold_Current_Min'];
            $DB_DEVICE_THRESHOLD_CURRENT_MAX = $ITEMS['Device_Threshold_Current_Max'];
			$DB_DEVICE_PHOTO = $ITEMS['Device_Photo'];
			
		?>
			<tr>
				<td style="text-align:center;">
					<input name="data[]" value="<?=$DB_DEVICE_ID?>" type="checkbox" />
				</td>
				</form>
				<td style="width: 100px; text-align: center;">
					<img width="70" height="70" src="device_image/<?=$DB_DEVICE_PHOTO?>">	
				</td>
                <td style="text-align: center;"><?php echo $DB_DEVICE_ID;?></td>
				<td style="text-align: center;"><?php echo $DB_DEVICE_LOCATION;?></td>
				<form action="Update Threshold Current" method="POST">
			    <td style="text-align: center;">
					<style>.centered-input {text-align: center;}</style>
			        <input type='text' class="centered-input"  style='width: 80px;'  name='Threshold_Current_Min' value='<?php echo $DB_DEVICE_THRESHOLD_CURRENT_MIN;?>'>
				</td>
                                    <td style="text-align: center;">
                                        <style>.centered-input {text-align: center;}</style>
                                        <input type='text' class="centered-input"  style='width: 80px;'  name='Threshold_Current_Max' value='<?php echo $DB_DEVICE_THRESHOLD_CURRENT_MAX;?>'>
                                    </td>
                                    <td style="text-align: center;">
                                        <button class="btn btn-primary btn-sm" type="submit" name="Update_btn_id" value='<?php echo $DB_DEVICE_ID;?>' >Set</button>
                                    </td>
